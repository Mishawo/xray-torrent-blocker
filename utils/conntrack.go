...
func (cm *ConntrackManager) DropConnections(ip string) error {
	...
	// PERFORMANCE FIX: Use the conntrack CLI tool instead of dumping the whole table.
	// This prevents massive CPU/RAM spikes when blocking multiple IPs.
	cmd := exec.Command("conntrack", "-D", "-s", ip)
	output, err := cmd.CombinedOutput()
	if err != nil {
		// It's normal for conntrack -D to return an error if no flows exist for the IP
		log.Printf("Conntrack drop for %s finished (may have no active flows): %v. Output: %s", ip, err, string(output))
	} else {
		log.Printf("Successfully dropped connections for IP %s via conntrack CLI", ip)
	}
...
func (cm *ConntrackManager) GetConnectionCount(ip string) (int, error) {
	...
	// PERFORMANCE FIX: Use CLI to count connections to avoid memory spikes
	cmd := exec.Command("conntrack", "-L", "-s", ip)
	output, err := cmd.Output()
	if err != nil {
		return 0, fmt.Errorf("failed to query conntrack: %v", err)
	}

	// Count the number of lines returned (each line is a flow)
