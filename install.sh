...
# --- download & run interactive helper script (inject_config.sh) ---
INJECT_SCRIPT_PATH="/opt/tblocker/inject_config.sh"
print_info "Downloading configuration helper..."
if curl -fsSL "https://raw.githubusercontent.com/MasterHide/injecting/main/inject_config.sh" -o "$INJECT_SCRIPT_PATH"; then
  chmod +x "$INJECT_SCRIPT_PATH"
  print_info "Launching configuration helper..."
  # Run it interactively (will present the menu)
  "$INJECT_SCRIPT_PATH"
else
  print_warning "Failed to download inject_config.sh. You can run it later with:"
  print_info "curl -O https://raw.githubusercontent.com/MasterHide/injecting/main/inject_config.sh && chmod +x inject_config.sh && ./inject_config.sh"
fi
# --- end helper download/run ---
...
echo ""
print_info "For additional parameters (webhooks, whitelist, etc.), use the 'tblock' command."
echo "==============================================================="
